...
static ACPI_STATUS resource_report_callback(ACPI_RESOURCE* res, void* _ctx) {
  auto* ctx = static_cast<struct ResourceContext*>(_ctx);
  zx_status_t status;
...
static zx_status_t read_mcfg_table(std::vector<McfgAllocation>* mcfg_table) {
  ...
  if (table_bytes % sizeof(acpi_mcfg_allocation)) {
...
zx_status_t pci_root_host_init() {
  ...
  zx_status_t st = RootHost.Init(get_root_resource());
  if (st != ZX_OK) {
    return st;
  }
