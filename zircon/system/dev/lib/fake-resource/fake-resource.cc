...
bool exclusive_region_overlaps(zx_rsrc_kind_t kind, zx_paddr_t new_rsrc_base,
  ...
  FakeHandleTable().ForEach(HandleType::RESOURCE, [&](Object* obj) -> bool {
    ...
    if (rsrc->kind() == kind && rsrc->is_exclusive()) {
      // Verify that the start or end of the new resource doesn't sit within the
      // existing exclusive resource's range
      zx_paddr_t rsrc_end = rsrc->base() + rsrc->size();
      if ((new_rsrc_base >= rsrc->base() && new_rsrc_end < rsrc_end) ||
          (new_rsrc_end > rsrc->base() && new_rsrc_end <= rsrc_end)) {
        overlaps = true;
        return false;
      }
    }
