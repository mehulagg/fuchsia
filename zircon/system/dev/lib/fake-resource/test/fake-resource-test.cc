...
class FakeResource : public zxtest::Test {
 public:
  zx_handle_t root_resource() { return root_; }
...
TEST_F(FakeResource, ChildBoundsTest) {
  ...
  ASSERT_OK(zx_resource_create(root_resource(), ZX_RSRC_KIND_MMIO, PAGE_SIZE, PAGE_SIZE,
                               parent_name.data(), parent_name.size(), &parent));
  ASSERT_TRUE(
      validate_resource_info(parent, PAGE_SIZE, PAGE_SIZE, ZX_RSRC_KIND_MMIO, parent_name.data()));
  zx_handle_t child;
  // Same span.
  ASSERT_OK(zx_resource_create(parent, ZX_RSRC_KIND_MMIO, PAGE_SIZE, PAGE_SIZE, child_name.data(),
                               child_name.size(), &child));
  // Before parent base.
  ASSERT_NOT_OK(zx_resource_create(parent, ZX_RSRC_KIND_MMIO, 0, PAGE_SIZE, child_name.data(),
                                   child_name.size(), &child));
  // Past parent length.
  ASSERT_NOT_OK(zx_resource_create(parent, ZX_RSRC_KIND_MMIO, PAGE_SIZE, PAGE_SIZE * 2,
                                   child_name.data(), child_name.size(), &child));
  // Superset of parent.
  ASSERT_NOT_OK(zx_resource_create(parent, ZX_RSRC_KIND_MMIO, 0, PAGE_SIZE * 2, child_name.data(),
                                   child_name.size(), &child));
