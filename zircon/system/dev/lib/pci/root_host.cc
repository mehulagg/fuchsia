...
// signal that was generated by the eventpair death.
//
...
void PciRootHost::WorkerThreadEntry() {
  while (true) {
    ...
    zx_status_t st = lifetime_port_.wait(zx::time::infinite(), &packet);
    if (st == ZX_OK) {
      switch (packet.type) {
        // The parent thread uses user signals to notify to exit
        case ZX_PKT_TYPE_USER:
          switch (packet.user.u32[0]) {
            case PciRootHost::Messages::kSyn: {
              const zx_port_packet_t ack_packet = {.type = ZX_PKT_TYPE_USER,
                                                   .user.u32[0] = PciRootHost::Messages::kAck};
              ack_port_.queue(&ack_packet);
              break;
            }
            case PciRootHost::Messages::kExit:
              zxlogf(TRACE, "%s received exit signal\n", kWorkerTag);
              return;
            default:
              zxlogf(ERROR, "%s Unexpected signal type to worker: %d\n", kWorkerTag,
                     packet.user.u32[0]);
          }
          break;
...
zx_status_t PciRootHost::SendMessageToWorker(const PciRootHost::Messages msg) {
  zx_port_packet_t packet = {.type = ZX_PKT_TYPE_USER, .user.u32[0] = msg};
  zx_status_t st = lifetime_port_.queue(&packet);
  if (st != ZX_OK) {
    zxlogf(ERROR, "%s Failed to send msg %#x to child thread: %d\n", kMainTag, msg, st);
  }
  return st;
}

// Send a packet ot the worker and wait for it to acknowledge it to ensure it has
// handled any eventpair signals in its port queue.
zx_status_t PciRootHost::WaitForWorkerAck() {
  zx_status_t st = SendMessageToWorker(PciRootHost::Messages::kSyn);
  if (st != ZX_OK) {
    return st;
  }

  zx_port_packet_t in;
  st = ack_port_.wait(zx::time::infinite(), &in);
  if (st != ZX_OK) {
    return st;
  }
  zxlogf(TRACE, "%s received ack\n", kMainTag);

  if (in.type != ZX_PKT_TYPE_USER || in.user.u32[0] != PciRootHost::Messages::kAck) {
...
zx_status_t PciRootHost::AllocateWindow(AllocationType type, zx_paddr_t base, size_t size,
  ...
  st = zx_resource_create(root_resource_, rsrc_kind | ZX_RSRC_FLAG_EXCLUSIVE, region_uptr->base,
                          region_uptr->size, name.data(), name.size(),
...
zx_status_t PciRootHost::RecordAllocation(RegionAllocator::Region::UPtr region,
  ...
  st =
      root_host_endpoint.wait_async(lifetime_port_, key, ZX_EVENTPAIR_PEER_CLOSED, 0 /* options */);
  if (st != ZX_OK) {
...
zx_status_t PciRootHost::Init(zx_handle_t root_resource) {
  assert(!initialized_);

  zx_status_t st = zx::port::create(0, &lifetime_port_);
  if (st != ZX_OK) {
    return st;
  }

  st = zx::port::create(0, &ack_port_);
  if (st != ZX_OK) {
    return st;
  }
...
PciRootHost::~PciRootHost() {
  SendMessageToWorker(PciRootHost::Messages::kExit);
  lifetime_thrd_.join();
  fflush(stdout);
}
